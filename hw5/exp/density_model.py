...
class Histogram(Density_Model):
    ...
    def update_count(self, state, increment):
        ...
        n_times = self.hist.get(bin_name)
        if n_times:
            self.hist[bin_name]=n_times+increment
        else:
    ...
    def get_count(self, states):
        ...
        """
        counts = []
        list_states = [states[i] for i in range(states.shape[0])]
        for state in list_states:
            bin_name = self.preprocessor(state)
            counts.append(self.hist.get(bin_name))
    ...
    def get_prob(self, states):
        ...
        for state in list_states:
            bin_name = self.preprocessor(state)
            count = self.hist.get(bin_name)
            probs.append(count / float(self.total))
        return probs
...
class RBF(Density_Model):
    ...
    def get_prob(self, states):
        ...
        if self.means is None:
            ...
            # 1. Compute deltas
            deltas = np.array([states[i] - self.means for i in range(b)])
            assert deltas.shape == (b, B, ob_dim)

            # 2. Euclidean distance
            euc_dists = np.sum(deltas**2, 2)
...
class Exemplar(Density_Model):
    ...
    def build_computation_graph(self):
        ...
        self.kl = tf.squeeze(self.encoder1.kl_divergence(self.prior) + self.encoder2.kl_divergence(self.prior))
    ...
    def make_encoder(self, state, z_size, scope, n_layers, hid_size):
        """
            ...
            Hint: use build_mlp
        """
        z_mean = build_mlp(state, int(z_size), scope, n_layers, hid_size)
        z_logstd = tf.get_variable('log_std', initializer=np.zeros((int(z_size), ), dtype=np.float32), dtype=np.float32)
    ...
    def make_prior(self, z_size):
        ...
        prior_logstd = np.ones((int(z_size), ), dtype=np.float32)
    ...
    def make_discriminator(self, z, output_size, scope, n_layers, hid_size):
        """
            ...
            Hint: use build_mlp
        """
        logit = build_mlp(z, int(output_size), scope, n_layers, hid_size)
    ...
    def get_likelihood(self, state1, state2):
        ...
        target = np.sum(state1 == state2, axis=1, keepdims=True)
